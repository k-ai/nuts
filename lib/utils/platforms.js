...
function satisfiesPlatform(platform, list) {
    ...
    // try fallback to 32 version.
    if ( platform.endsWith('_64') ) {
      const platform32 = platform.replace('_64', '_32');
      if (_.contains(list, platform32)) return true;
    }
...
function resolveForVersion(version, platformID, opts) {
    ...
    return _.chain(version.platforms)
        .filter(function(pl) {
            if ( pl.type.indexOf(platformID) === 0 ) {
              return true;
            }

            // try fallback to 32 version.
            if ( platformID.endsWith('_64') ) {
              const platform32 = platformID.replace('_64', '_32');
              if (pl.type === platform32) return true;
            }
