...
function Nuts(opts) {
    ...
    this.opts = _.defaults(opts || {}, {
        ...
        // defaultChannel for downloads without channel.
        defaultChannel: '*'
    });

    var channelsMask = this.opts.channelsMask;
    if ( channelsMask ) {
      channelsMask = JSON.parse(channelsMask);
      if ( !Array.isArray(channelsMask) ) {
        throw new Error('Channels mask should be an array!');
      }
    }
    ...
    this.versions = new Versions(this.backend, channelsMask);
    ...
    this.router.get('/download/channel/:channel/:platform/:filename', this.onDownload);
    ...
    this.router.get('/electron-updater/:channel/latest-mac.yml', this.onElectronUpdateMac);
...
Nuts.prototype.onDownload = function(req, res, next) {
    var that = this;
    var channel = req.params.channel || this.opts.defaultChannel;
...
Nuts.prototype.onElectronUpdate = function(channel, platform, filetype) {
    ...
    .then(function(channelObj) {
      ...
      .then(function(version) {
          ...
          // Extract sha2 file content and append with asset name.
          return that.backend.getAssetSha2(asset)
          .then(function(sha2) {
            return {
              version: version.tag,
              releaseDate: version.published_at,
              path: ['/download', 'channel', channel, platform, asset.raw.name].join('/'),
              sha2: sha2,
            };
          });
      });
